...
SELECT customers.email FROM customers JOIN orders ON customers.id = orders.customer_id GROUP BY customers.email HAVING SUM(num_cupcakes) > 100 ORDER BY customers.email DESC;
...
SELECT DISTINCT fname, lname, email FROM customers INNER JOIN orders ON customers.id = orders.customer_id INNER JOIN cupcakes ON orders.cupcake_id = cupcakes.id WHERE orders.processed = TRUE  AND cupcakes.name = 'funfetti'; 